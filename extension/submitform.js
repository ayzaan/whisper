...
document.addEventListener('DOMContentLoaded', function() {
	...
	$("#btn-addFriend").click(function ()
	{
		$.getJSON("http://www.projectvoid.com/whisper/whisper_controller.php?action=addFriend&recipientName="+$("#inputFriendName").get(0).value+"&groupid="+$("#groupName").get(0).value+"&key="+chrome.storage.local.get("keys")[$("#groupName").get(0).value], function(data) {return;});
	});
...
i[k&255]^d[p++],v=e[o>>>24]^f[n>>>16&255]^h[k>>>8&255]^i[m&255]^d[p++],n=e[n>>>24]^f[k>>>16&255]^h[m>>>8&255]^i[o&255]^d[p++],k=s,m=u,o=v;s=(g[k>>>24]<<24|g[m>>>16&255]<<16|g[o>>>8&255]<<8|g[n&255])^d[p++];u=(g[m>>>24]<<24|g[o>>>16&255]<<16|g[n>>>8&255]<<8|g[k&255])^d[p++];v=(g[o>>>24]<<24|g[n>>>16&255]<<16|g[k>>>8&255]<<8|g[m&255])^d[p++];n=(g[n>>>24]<<24|g[k>>>16&255]<<16|g[m>>>8&255]<<8|g[o&255])^d[p++];a[b]=s;a[b+1]=u;a[b+2]=v;a[b+3]=n},keySize:8});p.AES=h._createHelper(i)})();
		...
		$.getJSON("http://www.projectvoid.com/whisper/whisper_controller.php?action=newGroup&groupname="+$("#inputGroupName").get(0).value, function(data)
		{
			if (data['success'])
			{
				...
				chrome.storage.local.set({"keys":{'1' : "7de7aaf36c9d9170e325ff2aa0757e5e"}});
			}
		})
	});
});
